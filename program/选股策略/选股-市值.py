...
df2 = df2[df2['下日_是否交易'] == 1]
df2 = df2[df2['下日_开盘涨停'] == False]
df2 = df2[df2['下日_是否ST'] == False]
df2 = df2[df2['下日_是否退市'] == False]

# 计算选股因子，根据选股因子对股票进行排名
df['排名'] = df.groupby('交易日期')['总市值'].rank(ascending=True, method='first')
df2['排名'] = df2.groupby('交易日期')['总市值'].rank(ascending=True, method='first')


# 选取排名靠前的股票
df = df[df['排名'] <= select_stock_num]
df2 = df2[df2['排名'] <= select_stock_num]
print('下次选股：', df2)
