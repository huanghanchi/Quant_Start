...
# ===读取所有股票代码的列表
path = r'C:\Users\SpiceeYJ\Desktop\量化投资\网课代码\data\选股策略\xbx_stock_day_data\stock'
...
def calculate_by_stock(code):
    ...
    df['每天振幅'] = df['最高价'] / df['最低价'] - 1
    df['过去20日振幅'] = df['最高价'].rolling(20, min_periods=1).max() / df['最低价'].rolling(20, min_periods=1).min() - 1
    df['过去20日每日振幅均值'] = df['每天振幅'].rolling(20, min_periods=1).mean()
...
# d = calculate_by_stock('sz300736')
# print(d)
# exit()
# ===普通的办法
calculate_by_stock('sh600000')
...
all_stock_data.to_hdf(
    r'C:\Users\SpiceeYJ\Desktop\量化投资\网课代码\data\选股策略\all_stock_data_' + period_type + '.h5', 'df',
    mode='w')
