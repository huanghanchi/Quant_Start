...
# ===导入数据
# 从hdf文件中读取整理好的所有股票数据
df = pd.read_hdf(r'C:\Users\SpiceeYJ\Desktop\量化投资\网课代码\data\选股策略\all_stock_data_W.h5', 'df')
...
df['全部个股下周期涨跌幅'] = df['下周期每天涨跌幅'].apply(lambda x: np.cumprod(np.array(list(x))+1))
df['全部个股下周期涨跌幅'] = df['全部个股下周期涨跌幅'].apply(lambda x: x[-1])
df['全部个股本周期涨跌幅'] = df['本周期每天涨跌幅'].apply(lambda x: np.cumprod(np.array(list(x))+1))
df['全部个股本周期涨跌幅'] = df['全部个股本周期涨跌幅'].apply(lambda x: x[-1])

df['排名'] = df.groupby('交易日期')['全部个股本周期涨跌幅'].rank(ascending=True, method='first')  # True第一名涨幅最低,False反之
...
#print(select_stock['选股下周期涨跌幅'].shape)
# 计算下周期每天的涨跌幅
...
# ===计算选中股票每天的资金曲线
# 计算每日资金曲线
index_data = import_index_data(r'C:\Users\SpiceeYJ\Desktop\量化投资\网课代码\data\选股策略\sh000001.csv')
equity = pd.merge(left=index_data, right=select_stock[['交易日期', '买入股票代码']], on=['交易日期'],
                  how='left', sort=True)  # 将选股结果和大盘指数合并
#print(equity['选股下周期每天涨跌幅'].str.contains('NaN'))
#print(equity)
