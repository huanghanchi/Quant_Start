...
# =====需要手工调整的参数
stock_code = 'sh601258'  # 监控的股票，也可以是ETF
...
# =====初始化同花顺自动交易系统
ths = et.use('ths')
ths.connect(r'C:\同花顺软件\同花顺\xiadan.exe')  # 填入交易客户端的路径。即安装目录下的 xiadan.exe 的地址
ths.clear()  # 清理不必要的弹窗
...
def main():
    ...
    if sell_stock:
        ...
        # sell_amount = 100
        order_info = xbx_sell(ths, code=stock_info['交易代码'], price=order_price, amount=sell_amount)
    ...
    if buy_stock:
        ...
        if buy_amount >= 100:
            # 下单
            order_info = xbx_buy(ths, code=stock_info['交易代码'], price=order_price, amount=buy_amount)
